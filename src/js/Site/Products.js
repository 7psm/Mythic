// =============================================
//        PAGE PRODUITS - MythicMarket
// =============================================
import { updateCartCount, formatPrice } from '../utils.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const products = [
    {
      ...
      image: '',
      badge: 'LEGAL PAID',
      options: [{ name: 'Nitro Boost 1 mois', price: 6.00 }]
    },
    {
      id: 2,
      ...
      image: '',
      badge: 'LEGAL PAID',
      options: [{ name: 'Nitro Basic 1 mois', price: 1.50 }]
    },
    {
      id: 3,
      ...
      image: '',
      badge: 'LEGAL PAID',
      options: [{ name: 'Server Boost 1 mois x14', price: 15.00 }]
    },
    { 
      id: 4,
      ...
      image: '',
      badge: 'LEGAL PAID',
      options: [{ name: 'Server Boost 1 mois x17', price: 19.00 }]
    },
    {
      id: 5,
      ...
      badge: 'LEGAL PAID',
      options: [{ name: 'Server Boost 1 mois x20', price: 24.00 }]
    },
    {
      id: 6,
      ...
      image: '',
      badge: 'LEGAL PAID',
      options: [{ name: 'Server Boost 3 mois x14', price: 30.00 }]
    },
    {
      id: 7,
      ...
      image: '',
      badge: 'LEGAL PAID',
      options: [{ name: 'Server Boost 3 mois x17', price: 34.00 }]
    },
    {
      id: 8,
      ...
      image: '',
      badge: 'LEGAL PAID',
      options: [{ name: 'Server Boost 3 mois x20', price: 38.00 }]
    },
    {
      id: 9,
  ...
  const categoriesConfig = {
    nitro: { name: 'Discord Nitro', icon: '⚡', gradient: 'from-[#5865f2] to-[#7289da]' },
    boost: { name: 'Server Boost', icon: '🚀', gradient: 'from-[#f093fb] to-[#f5576c]' },
    decoration: { name: 'Avatar Decorations', icon: '✨', gradient: 'from-[#43e97b] to-[#38f9d7]' },
    members: { name: 'Discord Members', icon: '👥', gradient: 'from-[#fa709a] to-[#fee140]' },
    accounts: { name: 'Accounts', icon: '🎵', gradient: 'from-[#30cfd0] to-[#330867]' }
  };

  // =============================================
  //        GRADIENTS POUR LES PRODUITS
  // =============================================
  ...
  function renderProducts() {
    ...
    Object.keys(categoriesConfig).forEach(key => {
      categories[key] = { ...categoriesConfig[key], products: [] };
    });
    ...
    // Afficher chaque catégorie avec sa bannière
  ...
  function createProductCard(product) {
    ...
    const originalPrice = product.originalPrice ? 
      `<span class="text-sm text-discord-text-muted line-through ml-1">${formatPrice(product.originalPrice)}</span>` : '';
      ...
      <div class="p-4 bg-discord-bg">
        ...
        <div class="flex justify-between items-center mt-0 pt-0">
          <div class="flex flex-row items-baseline gap-1.5">
            <span class="text-lg font-bold text-discord-text">${formatPrice(product.price)}${discountText}</span>
            ${originalPrice}
          </div>
        </div>
      </div>
  ...
  function openProductModal(product) {
    if (!productModal || !modalBody) {
      console.warn('⚠️ Éléments modal introuvables, impossible d\'ouvrir la modal');
      return;
    }
    ...
    const bgGradient = modalGradients[product.id % modalGradients.length];

    // Générer le HTML des options si plusieurs options disponibles
    let optionsHTML = '';
        ...
        <div class="mb-6">
          ...
          <select id="product-option-select" class="w-full bg-[#1a1a1a] border border-white/20 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#667eea] focus:ring-2 focus:ring-[#667eea]/50 transition-all cursor-pointer hover:border-white/30 appearance-none" style="background-image: url('data:image/svg+xml;charset=UTF-8,%3csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 24 24%27 fill=%27none%27 stroke=%27white%27 stroke-width=%272%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27%3e%3cpolyline points=%276 9 12 15 18 9%27%3e%3c/polyline%3e%3c/svg%3e'); background-repeat: no-repeat; background-position: right 0.7rem center; background-size: 1.2em; padding-right: 2.5rem;">
            ${product.options.map((option, index) => `
              <option value="${index}" data-price="${option.price}" style="background-color: #1a1a1a; color: white; padding: 10px;">${option.name} - ${formatPrice(option.price)}</option>
            `).join('')}
          </select>
        </div>
      `;
        ...
        <div class="relative lg:w-2/5 bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] p-8 lg:p-10 flex flex-col justify-between order-2 lg:order-1">
          <div class="flex-1">
            ...
            <div class="flex flex-wrap gap-2 mb-6">
              ...
              <span class="px-3 py-1.5 bg-purple-500/10 border border-purple-500/20 rounded-lg text-purple-400 text-xs font-semibold">✓ Paiement sécurisé</span>
          ...
          <div>
            <div class="relative mb-6 p-[2px] rounded-2xl bg-gradient-to-r from-[#667eea] via-[#764ba2] to-[#667eea] animate-gradient-xy">
              <div class="bg-[#1a1a1a] rounded-2xl p-5">
                <div class="flex items-baseline gap-3 mb-3">
                  <span id="modal-price-display" class="text-white text-5xl font-black tracking-tight">${formatPrice(basePrice)}</span>
                  ${product.originalPrice ? `<span class="text-gray-500 text-xl line-through">${formatPrice(product.originalPrice)}</span>` : ''}
                </div>
                ...
                <div class="nitro-price-container relative group cursor-help">
                  <div class="flex items-center gap-2 text-sm">
                    ...
                    <span class="text-gray-400 font-medium">${formatPrice(nitroPrice)} <span class="text-[#5865F2] font-semibold">Avec ClientCare+</span></span>
                  </div>
            ...
            <button class="modal-buy-button group relative w-full bg-gradient-to-r from-[#667eea] to-[#764ba2] hover:from-[#764ba2] hover:to-[#667eea] text-white px-8 py-4 rounded-xl font-bold text-lg transition-all duration-300 overflow-hidden shadow-2xl hover:shadow-[0_0_40px_rgba(102,126,234,0.6)] hover:scale-[1.02] active:scale-[0.98]" data-product-id="${product.id}">
              <span class="relative z-10 flex items-center justify-center gap-3">
                ...
                <span id="modal-buy-text">Acheter pour ${formatPrice(basePrice)}</span>
              </span>
    ...
    if (optionSelect) {
      optionSelect.addEventListener('change', (e) => {
        ...
        const nitroPriceEl = document.getElementById('modal-nitro-price');
        const buyTextEl = document.getElementById('modal-buy-text');
        
        if (priceDisplay) priceDisplay.textContent = formatPrice(newPrice);
        if (nitroPriceEl) nitroPriceEl.textContent = formatPrice(newNitroPrice);
        if (buyTextEl) buyTextEl.textContent = `Acheter pour ${formatPrice(newPrice)}`;
      });
    }
    ...
    toast.innerHTML = `
      <div class="relative p-4 flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-white/90 text-sm font-semibold mt-1">${quantity}x ${formatPrice(price)}</div>
        </div>
  ...
  //        INITIALISATION
  // =============================================
  // Vérifier si on est sur la page Products avant d'initialiser
  if (productsGrid) {
    filterProducts();
    renderProducts();
  }
  
  // updateCartCount peut être appelé sur toutes les pages
  updateCartCount();
});
